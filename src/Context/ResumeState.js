...
const ResumeState = (props) => {
  ...
  const handlePrint = useReactToPrint({
    ...
    onAfterPrint: () => {
      setLoading(false);
      componentRef.current.style.padding = "10px";
    },
  });
  ...
  const initialData = {
    ...
    certificateData: {
      certificateTitles: { cTitle1: "Certificate Title 1" },
      certificateDesc: { cDescription1: "Certificate Description 1" },
    },
    workData: {
  ...
  const [checkHobbie,setCheckHobbie] = useState(false)
  const [checkLanguages,setCheckLanguages] = useState(false);
  ...
  return (
    <ResumeContext.Provider
      value={{
        ...
        checkCertificate,
        setCheckCertificate,
        checkLinkedin,setCheckLinkedin,
        checkGithub,setcheckGithub
